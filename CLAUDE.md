...
6. 老板可见回复必须保留 Go MCP 返回的 `source_note` 和 `source_update_note`。老板可见最终回答必须输出来源和来源更新时间；这两项是事实原子，只补来源两行即可，不要求把整个 `final_answer` 原样复制成固定模板。财务来源文件名和更新时间只认 `fin_file_mappings`，没有映射就不要用表注释、历史文件名或记忆兜底；合同和发票来源分别来自 `contract_main`、`contract_invoices`。
...
0. 对任何财务、经营、合同、回款、开票、收入、成本、利润、现金、银行、税额、应收/应付、客户、供应商或来源表问题，必须先调用 Go MCP，再回答。推荐命令：
   ...
   printf '%s\n%s\n' \
     ...
     | financeqa serve
   ```
   若当前环境没有全局 `financeqa`，先在仓库内执行 `go build -o bin/financeqa ./cmd/financeqa/...`，再用 `./bin/financeqa serve`。线上固定且唯一的二进制路径是 `~/finance_qa/bin/financeqa`。解析 `tools/call` 返回的 `content[0].text` JSON 后，如果存在 `final_answer`，必须优先以它为权威结果；其次才用 `boss_reply_text`、`boss_reply`、`message`。老板可见回复必须保留 Go MCP 结果里的关键数值、期间、业务口径、来源和来源更新时间，可以按老板汇报风格重写措辞，但不能换算、改口径、删来源，不能用历史对话、记忆、旧答案、利润表/银行流水/原始 SQL 自己重算替代 Go MCP 的最终答案。
...
11. 不能绕过后端实体候选和置信度，自己把修饰词、指标词、时间词或问题片段当客户/供应商/项目。
12. 不能把 `source_cell_notes` / `remarks` 当金额合计来源；它们是备注和审计解释材料，只有问题需要备注语义时才使用。
