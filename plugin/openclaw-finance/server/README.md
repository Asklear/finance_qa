...
为避免模型偶发漏掉老板可见来源，finance extension 还会在 `before_message_write` 做窄范围补强：仅当同一会话刚产生当前 `finance-query` 结果，且最终 assistant 文本缺少 `source_note/source_update_note` 时，追加 `来源` 和 `来源更新时间` 两行。这个逻辑不复制完整 `final_answer`，也不处理其他插件或其他 MCP 的回答。
