...
func Bootstrap(ctx context.Context, dbPath string) error {
	if looksLikeSQLitePath(strings.TrimSpace(dbPath)) {
		...
		sqliteColumnUpgrades := []struct {
			...
			{tableName: "contract_main", columnName: "feishu_modified_time", columnType: "TEXT"},
			...
			{tableName: "contract_invoices", columnName: "feishu_modified_time", columnType: "TEXT"},
	...
	ddls := []string{
		...
		fmt.Sprintf(`ALTER TABLE IF EXISTS %s.contract_main ADD COLUMN IF NOT EXISTS feishu_modified_time TEXT`, schema),
		...
		fmt.Sprintf(`ALTER TABLE IF EXISTS %s.contract_invoices ADD COLUMN IF NOT EXISTS feishu_modified_time TEXT`, schema),
