...
func buildContractAggregateSupplement(selection contractAggregateSelection, summary contractAggregateSummary) string {
	...
	case selection.IncludeRevenue && !selection.IncludeCost && !selection.IncludeProfit:
		return fmt.Sprintf("补充项目现金到账 %.2f 元，已开票 %.2f 元。%s", round2(summary.RevenueReceived), round2(summary.RevenueInvoiced), buildRevenueDetailSentence(summary.RevenueItems))
	case selection.IncludeReceivable && !selection.IncludePayable:
		return fmt.Sprintf("补充项目结算 %.2f 元、已到账 %.2f 元；其中已开票未回款 %.2f 元。", round2(summary.RevenueSettlement), round2(summary.RevenueReceived), round2(summary.RevenueInvoiceOpen))
