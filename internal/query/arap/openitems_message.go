...
import (
	...
	queryorchestration "financeqa/internal/query/orchestration"
)

func FormatOpenItemSummaryMessage(period, account, entity string, summary openitems.Summary, historyLabel, currentLabel string) string {
	scopeLabel := account
	...
	if len(settlementParts) > 0 {
		msg += "，其中" + queryorchestration.JoinWithComma(settlementParts)
	}
	msg += "）"
	return msg
}

func BuildOpenItemMessage(data map[string]any) string {
	typ := queryorchestration.AnyToString(data["type"])
	historyLabel := "历史应收"
	currentLabel := "当月新增应收"
	if typ == "payable" {
		historyLabel = "历史应付"
	...
	if raw, ok := data["settlement_confidence"].(map[string]any); ok {
		confidence = openitems.SettlementConfidence{
			ConfirmedHistoricalSettlement: toFloat64(raw["confirmed_historical_settlement"]),
			ProbableHistoricalSettlement:  toFloat64(raw["probable_historical_settlement"]),
			ConfirmedCurrentSettlement:    toFloat64(raw["confirmed_current_settlement"]),
			ProbableCurrentSettlement:     toFloat64(raw["probable_current_settlement"]),
			UnmatchedDecrease:             toFloat64(raw["unmatched_decrease"]),
		}
	}

	return FormatOpenItemSummaryMessage(
		queryorchestration.AnyToString(data["period"]),
		queryorchestration.AnyToString(data["account"]),
		queryorchestration.AnyToString(data["entity"]),
		openitems.Summary{
			ClosingBalance:       toFloat64(data["total"]),
			OpeningBalance:       toFloat64(data["opening_balance"]),
			CurrentIncrease:      toFloat64(data["current_increase"]),
			CurrentDecrease:      toFloat64(data["current_decrease"]),
			HistoricalSettlement: toFloat64(data["historical_settlement"]),
			CurrentSettlement:    toFloat64(data["current_period_settlement"]),
