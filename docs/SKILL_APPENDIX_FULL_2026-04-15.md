...
1. 总量型核心指标问题（收入/成本/利润/销售额）默认优先返回财务确认口径结果。
...
1. 月度收入/成本/利润（默认财务确认口径；明确追问现金流/到账/差异时再展开现金视角）。
...
9. 某月销项税额。
10. 某月进项税额。
11. 某月总成本（默认走财务确认口径；必要时补充现金支出口径）。
12. 某月应收账款（余额表口径）。
13. 某月应付账款（余额表口径）。
14. 某项目应收/应付（项目净流入口径）。
...
当问题涉及总量型 `收入/成本/利润/销售额` 时，默认返回财务确认口径：

1. 直接使用利润表 / 财务确认结果组织回答。
2. 返回 `account_value` 与 `metrics` 作为主字段。
3. 为兼容旧调用方，仍保留现金流相关字段，但不在默认老板回答里强制展开。
...
1. 只有问题明确提到 `银行卡`、`到账`、`回款`、`付款`、`现金流` 或 `差异原因` 时，才展开“银行卡上看”解释。
2. 如果问题明确在问某个客户、供应商、员工或项目，优先返回主体审计结果，不强制改成双视角汇总。
...
2. 业务解释：默认说明财务确认口径；只有问题明确提到现金流、到账、付款或差异原因时，再补“银行卡上看/卡上实际进出账”解释。
...
1. `结论：{时间}公司账上确认收入{D}、成本{E}、利润{F}。`
2. `如果继续看现金流，本月实际到账{A}、实际花出{B}，差异主要因为{原因}。`
...
1. 不要把”收入”直接等同”银行到账”。
2. 不要把”成本”直接等同”银行支出”。
3. 涉及核心指标，默认走财务确认口径；只有明确问现金流或差异时再展开双视角。
