...
1. `appendix_doc_version`: `2026-04-21.1`
2. `skill_contract_version`: `2026-04-21.1`
...
1. 总量型核心指标问题（收入/成本/利润/销售额）默认先返回现金收付，再补经营确认结果。
...
2. 如果问题继续追问 `差异原因`、`为什么不一样`、`回款和利润差异`，再把利润桥、税项时差和预提/冲回影响讲透。
3. 如果问题明确在问某个客户、供应商、员工或项目，优先返回主体审计结果，不强制改成双视角汇总。
...
1. 当前 `finance_bridge.py` 只注册上述 3 个工具，不在桥接层读取或注入 `SKILL.md` / appendix 正文内容；但会读取 `SKILL.md` 顶部的契约版本标记，校验 appendix 相对路径存在，并把这些信息写入 `bridge_meta`。
